...
int main(int argc, char **argv, char **env)
{
  ...
  if (arg_jtag[0])
  {
    ...
    if (jtag->init_jtag_server(JTAG_VPI_SERVER_PORT, JTAG_VPI_USE_ONLY_LOOPBACK) != VerilatorJtagServer::SUCCESS)
    {
  ...
  while (!(done || Verilated::gotFinish()))
  {
    ...
    if (tfp)
      tfp->dump(main_time);
    if (baud_rate)
      do_uart(&uart_context, top->o_uart_tx);
